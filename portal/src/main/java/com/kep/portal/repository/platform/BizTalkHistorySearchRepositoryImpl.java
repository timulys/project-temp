...
public class BizTalkHistorySearchRepositoryImpl implements BizTalkHistorySearchRepository {
    ...
    @Override
    public List<BizTalkHistory> statisticsSuccessFail(ZonedDateTime from, ZonedDateTime to, Long branchId, Long teamId, Long memberId,PlatformType type) {
        QBizTalkHistory qBizTalkHistory = new QBizTalkHistory("bizTalkHistory");
        BooleanBuilder whereBuilder = new BooleanBuilder();

        if(branchId != null){
            whereBuilder.and(bizTalkHistory.branchId.eq(branchId));
        }

        if(teamId != null){
            whereBuilder.and(bizTalkHistory.teamId.eq(teamId));
        }

        if(memberId != null){
            whereBuilder.and(bizTalkHistory.creator.eq(memberId));
        }

        if(type != null){
            whereBuilder.and(bizTalkHistory.type.eq(type));
        }

        return queryFactory.select(
                        (Projections.fields(
                                BizTalkHistory.class
                                ,qBizTalkHistory.type
                                ,qBizTalkHistory.status
                                ,qBizTalkHistory.branchId
                                ,qBizTalkHistory.teamId
                                ,qBizTalkHistory.templateId
                                ,qBizTalkHistory.sendDate
                                ,qBizTalkHistory.creator

                        ))
                ).from(qBizTalkHistory)
                .where(qBizTalkHistory.sendDate.between(from , to).and(whereBuilder))
                .fetch();
    }
